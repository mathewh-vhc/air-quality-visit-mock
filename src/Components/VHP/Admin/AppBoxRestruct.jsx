...
import {MergeObject, ImageButton} from 'vhp-component-library';
...
export class AppBox extends Component {
    ...
    componentDidMount() {
        if (this.state.settings.dev == false) {
            //USER MODE SETUP\
			if (localStorage.getItem("user") != "" && localStorage.getItem("pswrd") != "") {
				let config = JSON.parse(JSON.stringify(this.state.config))
				config.user.name = localStorage.getItem("user")
				config.user.pswrd = localStorage.getItem("pswrd")
				this.setState({
					config: config
				})
				this.CreateCore(true)
			} else {
    ...
    CreateCore(autologin = false) {
		console.log("Creating a core")
        this.setState({
            API:new Core({dev:false})
        },()=>{
            if (autologin) {
		...
		this.state.API.Login({
			...
			answr=>{
				...
				if(answr == true){// GOOD
					...
					localStorage.setItem("pswrd", data.password)
					return true
				} else {
					if (!autologin) {
						this.setState({
